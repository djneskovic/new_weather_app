...
export default {
	...
	mutations: {
		SEARCH_FOR_CITY(state, payload) {
			...
			state.iconToday = payload.forecast.forecastday[0].day.condition.icon;
			state.iconTomorow = payload.forecast.forecastday[1].day.condition.icon;
			state.iconNextDay = payload.forecast.forecastday[2].day.condition.icon;
			state.degreeToday = payload.forecast.forecastday[0].day.maxtemp_c;
			state.degreeTomorow = payload.forecast.forecastday[1].day.maxtemp_c;
			state.degreeNextDay = payload.forecast.forecastday[2].day.maxtemp_c;
		...
		ADD_TO_FAVORITES(state) {
			...
			localStorage.setItem("favorites", JSON.stringify(state.favoritesCities));
		},
		...
		AUTOCOMPLETE_CITY(state, payload) {
			...
			data.forEach((find) => {
				...
				const uniqueCity = [...new Set(state.autocompleteCities.map((x) => x.id))];
				const newUniqueCity = uniqueCity.map((id) => state.autocompleteCities.find((x) => x.id === id));
	...
	actions: {
		searchCity({ state, commit, getters }) {
			...
			axios.get(`https://api.weatherapi.com/v1/forecast.json?key=c00704bfd174489e9e685223222812&q=${getters.getInputText}&days=5&aqi=no&alerts=no`)
				.then((res) => {
		...
		autocompleteCity({ state, commit, getters }) {
			...
			axios.get(`https://api.weatherapi.com/v1/search.json?key=c00704bfd174489e9e685223222812&q=${getters.getInputText}`)
				.then((res) => {
	...
	getters: {
		...
		bg(state) {
			if (state.degree < 10 && state.showForecast) return "coldWeather";
			if (state.degree >= 10 && state.showForecast) return "warmWeather";

			return "bg";
		},

		getFavorites(state) {
			return (state.favoritesCities = JSON.parse(localStorage.getItem("favorites") || "[]"));
		},
		...
		citiesWhichNotStartWithInput(state) {
			if (state.inputText != "" && state.inputText) {
				return state.autocompleteCities.filter((item) => !item.name.toLowerCase().includes(state.inputText.toLowerCase()));
			}
			return state.autocompleteCities;
		},
		...
		orderAutoComplete(_, getters) {
			return [...getters.citiesWhichStartWithInput, ...getters.citiesWhichNotStartWithInput];
		},
